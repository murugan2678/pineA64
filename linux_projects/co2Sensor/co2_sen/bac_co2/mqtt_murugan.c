...
{
  int fd = open(DATA_STORE_FILE, O_CREAT | O_RDWR, 0644);
  FILE *fp = fdopen(fd, "a");
  if (!fp)
  {
...
{
  int fd1 = open(DATA_STORE_FILE, O_RDWR, 0664);
  if (fd1 == -1)
  {
    perror("error open the fd1");
    exit(-1);
  }
  else
  {
    printf("sucessfully created fd1 :%d\n", fd1);
  }
