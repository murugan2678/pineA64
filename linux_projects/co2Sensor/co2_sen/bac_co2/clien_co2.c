...
void *co2_client_thread(void *arg)
{
  ...
  while(1)
  {
    ...
    if (num_read == -1)
    {
      log_error("CO2 Modbus read registers failed : %s", modbus_strerror(errno));
      printf("CO2 Modbus read registers failed : %s", modbus_strerror(errno));
      modbus_close(ctx);

      //notcnt1 = 1;

      /* uint16_t write_regs[NUM_OF_REGISTERS];
	 write_regs = {0};    // modbus failed means send to write zero 0 for all register. in server_ctx
	 if (notcnt1 == 1)
	 {
	...
	continue;
      ...
      modbus_write_registers(server_ctx, CO2_REGISTER_START, NUM_OF_REGISTERS, write_regs);
      sleep(10);
      continue;
    }
    ...
    if (num_read1 == -1)
    {
      ...
      /* uint16_t write_regs1[NUM_OF_REGISTERS];
      //write_regs1 = {0};    // modbus failed means send to write zero 0 for all register. in server_ctx
      if (notcnt1 == 1)
      {
