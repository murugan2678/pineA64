...
void *co2_client_thread(void *arg)
{
    ...
    // Modbus set slave ID for first device
    ...
    // Modbus set slave ID for second device
    ...
    // Modbus server connect (IP: 192.168.0.136, Port: 5503)
    ...
    while (1)
    {
        ...
        // Modbus read registers for first sensor data
        ...
        if (num_read == -1)
        {
            ...
            modbus_close(ctx);
            // Write zeros to server for index 1
            if (modbus_write_registers(server_ctx, CO2_REGISTER_START, NUM_OF_REGISTERS, write_regs) == -1)
            {
                ...
                printf("Failed to write zeroed registers to server (index 1)\n");
            }
            else
            {
        ...
        {
            // Process first sensor data (IP: 192.168.0.200)
            float raw = reg[0];
            ...
            // Write first device data to server
        ...
        // Modbus read registers for second sensor data
        ...
        if (num_read1 == -1)
        {
            ...
            modbus_close(ctx1);
            // Write zeros to server for index 2
        ...
        {
            // Process second sensor data (IP: 192.168.0.201)
            float raw6 = reg1[0];
            ...
            // Write second device data to server
