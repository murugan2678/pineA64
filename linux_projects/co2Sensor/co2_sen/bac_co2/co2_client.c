...
{
  // modbus *ctx max devices pass to array
  ...
  //  Initialize Modbus contexts two devices, total devices
  ...
  modbus_t *server_ctx = modbus_new_tcp(SERVER_ADDRESS, SERVER_PORT);    // modbus server for 192.168.0.136
  ...
  while (keep_running)   //  running data
  {
	...
	if (ctx[i] == NULL || modbus_set_slave(ctx[i], devices[i].device_slave_ID) != 0 || modbus_connect(ctx[i]) != 0) 
	{
	  ...
	  /**************** when modbus failed first device all register to zero *************************/

	  // Write zeros to registers for failed device, for device 1 register all when client modbus poll data. i 0 send to server
	  if (i == 0)                         
	  { 					// Device 1
	    ...
	    {
	      log_error("Failed to write zeros to registers 10-14");
	      printf("Failed to write zeros to registers 10-14\n");
	    }
	    else 
	    {
	      printf("Write zeros to registers 0-4 due to device %d failure\n", devices[i].id);
	    }
	  }

	  /**************** when modbus failed second device all register to zero *************************/

	  // Write zeros to registers for failed device, for device 2 register all when client modbus poll data. i 0 send to server
	  if (i == 1)                         
	  { 					// Device 2
	    uint16_t zero_regs[5] = {0};
	    ...
	    {
	      printf("Write zeros to registers 10-14 due to device %d failure\n", devices[i].id);
	    }
	  }
	  continue;
	}
	...
	if (i == 0) // Device 1
	{
	  ...
	  /* writer register to server */
	  /* modbus_write_register ---> write a single register */
	  //  int modbus_write_register(modbus_t *ctx, int addr, const uint16_t value);
	  if (modbus_write_registers(server_ctx, CO2_REGISTER_START, 5, zero_regs) == -1) 
	  {
	    log_error("Failed to write zeros to registers 10-14: %s", modbus_strerror(errno));
	    printf("Failed to write zeros to registers 10-14: %s\n", modbus_strerror(errno));
	    fflush(stdout);
	  } 
	  else
	  {
	    printf("Wrote zeros to registers 10-14 due to device %d failure\n", devices[i].id);
	    fflush(stdout);
	  }
	}
