...
void *co2_client_thread(void *arg)
{
    ...
    // Find CO2 devices (ID 1 and ID 2)
    ...
    // Modbus connect for second device (IP: 192.168.0.201)
    ...
    // Set slave IDs
    if (modbus_set_slave(ctx, devices[co2_device_index].device_slave_ID) == -1)
    {
        log_error("Invalid slave ID for CO2 index 1: %s", modbus_strerror(errno));
        perror("Invalid slave ID for CO2 index 1");
        modbus_free(ctx);
        return NULL;
    }

    if (modbus_set_slave(ctx1, devices[co2_device_index1].device_slave_ID) == -1)
    {
    ...
    // Set response timeout for both devices
    struct timeval timeout;
    timeout.tv_sec = 1; // 1 second timeout
    timeout.tv_usec = 0;
    modbus_set_response_timeout(ctx, timeout.tv_sec, timeout.tv_usec);
    modbus_set_response_timeout(ctx1, timeout.tv_sec, timeout.tv_usec);
    ...
    // Connect to server (IP: 192.168.0.136, Port: 5503)
    ...
    modbus_set_response_timeout(server_ctx, timeout.tv_sec, timeout.tv_usec);
    ...
    while (1)
    {
        ...
        if (num_read == -1)
        {
            ...
            printf("CO2 Modbus read registers failed index 1: %s\n", modbus_strerror(errno));
            modbus_close(ctx);

	    // modbus zero
            if (modbus_write_registers(server_ctx, CO2_REGISTER_START, NUM_OF_REGISTERS, write_regs) == -1)
            {
                log_error("Failed to write zeroed registers to server (index 1): %s", modbus_strerror(errno));
                printf("Failed to write zeroed registers to server (index 1): %s\n", modbus_strerror(errno));
            }
            else
            {
                printf("Wrote %d zeroed registers to server at CO2_REGISTER_START=%d (index 1)\n", NUM_OF_REGISTERS, CO2_REGISTER_START);
            }

            if (modbus_connect(ctx) == -1)
            {
                log_error("CO2 Modbus reconnect failed index 1: %s", modbus_strerror(errno));
                perror("CO2 Modbus reconnect failed index 1");
                sleep(10);
                continue;
            }
            sleep(10);
        ...
        else if (num_read > 0)
        {
            ...
            if (modbus_write_registers(server_ctx, CO2_REGISTER_START, NUM_OF_REGISTERS, write_regs) == -1)
            {
                log_error("Failed to write to server registers (index 1): %s", modbus_strerror(errno));
                printf("Failed to write to server registers (index 1): %s\n", modbus_strerror(errno));
            ...
            {
                printf("Wrote %d registers to server at CO2_REGISTER_START=%d (index 1)\n", NUM_OF_REGISTERS, CO2_REGISTER_START);
            }
       ...
       // memset(write_regs1, 0, sizeof(write_regs1)); // Initialize to zero
        ...
        if (num_read1 == -1)
        {
            ...
            printf("CO2 Modbus read registers failed index 2: %s\n", modbus_strerror(errno));

            modbus_close(ctx1);

           if (modbus_write_registers(server_ctx, CO2_REGISTER_START2, NUM_OF_REGISTERS, write_regs1) == -1)
            {
                log_error("Failed to write zeroed registers to server (index 2): %s", modbus_strerror(errno));
                printf("Failed to write zeroed registers to server (index 2): %s\n", modbus_strerror(errno));
            }
            else
            {
                printf("Wrote %d zeroed registers to server at CO2_REGISTER_START2=%d (index 2)\n", NUM_OF_REGISTERS, CO2_REGISTER_START2);
            } 
            if (modbus_connect(ctx1) == -1)
            {
                log_error("CO2 Modbus reconnect failed index 2: %s", modbus_strerror(errno));
                perror("CO2 Modbus reconnect failed index 2");
                sleep(10);
                continue;
            }
            sleep(10);
        ...
        else if (num_read1 > 0)
        {
            ...
            float raw6 = reg1[0];
            float ppm6 = (raw6 / 4095) * 2000; // CO2
            uint16_t ppm_scaled6 = (uint16_t)ppm6;
            float raw7 = reg1[1];
            float ppm7 = (raw7 / 4095) * 5000; // temperature
            uint16_t ppm_scaled7 = (uint16_t)ppm7;
            float raw8 = reg1[2];
            float ppm8 = (raw8 / 4095) * 100; // humidity
            uint16_t ppm_scaled8 = (uint16_t)ppm8;
            float raw9 = reg1[3];
            float ppm9 = (raw9 / 4095) * 600; // PM2.5
            uint16_t ppm_scaled9 = (uint16_t)ppm9;
            float raw10 = reg1[4];
            float ppm10 = (raw10 / 4095) * 500; // PM10
            uint16_t ppm_scaled10 = (uint16_t)ppm10;

            write_regs1[10] = ppm_scaled6;
            write_regs1[11] = ppm_scaled7;
            write_regs1[12] = ppm_scaled8;
            write_regs1[13] = ppm_scaled9;
            write_regs1[14] = ppm_scaled10;
	    ...
	    //if (modbus_write_registers(server_ctx, CO2_REGISTER_START2, 5, &write_regs1[10]) == -1)
            if (modbus_write_registers(server_ctx, CO2_REGISTER_START2, NUM_OF_REGISTERS, write_regs1) == -1)
            {
                log_error("Failed to write to server registers (index 2): %s", modbus_strerror(errno));
                printf("Failed to write to server registers (index 2): %s\n", modbus_strerror(errno));
                modbus_close(server_ctx);
                if (modbus_connect(server_ctx) == -1)
                {
                    log_error("Server Modbus reconnect failed: %s", modbus_strerror(errno));
            ...
            {
                printf("Wrote %d registers to server at CO2_REGISTER_START2=%d (index 2)\n", NUM_OF_REGISTERS, CO2_REGISTER_START2);
            }
