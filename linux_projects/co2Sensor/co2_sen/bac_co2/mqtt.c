...
void forward_stored_messages(MQTTClient *client) 
{
  ...
  while (fgets(line, sizeof(line), fp)) 
  {
    ...
    else 
    {
      ...
      log_data("Forwared stored message topic '%s': %s", topic, payload); 	// this for log_data print for log directory printing
    }
  }
...
int MQTT_Publish(MQTTClient *client, char *payload, char *topic) 
{
  ...
  log_data("Published to topic '%s': %s", topic, payload);
