 ...
 * Date        : 03-9-2024                                                     *
 * File        : co2_sen            		                               *
 * Description : co2 sensor and two sensor data                                *
 *               two sensor data reading to different IP for                   *
 *               waveshare modbus TCP/IP. this two ip fetch data store to      *
 *               single ip sote                                                *
 *                                                                             *
 *******************************************************************************/
...
#define MQTT_PASSWORD "Yrzp62Nu20H0"	// "UUOuKQvTSUtv"	// zedbee.io mqtt password  ---> UUOuKQvTSUtv
...
void *mqtt_thread(void *arg) 
{
   ...
   /*  char cwd[256];
	...
	// data send mqtt, createMainPkt_B this function for jsonify.c this files inside there. this data send to createMainPkt_B in jsonify.c
        createMainPkt_B(json_buffer, seq++, current_time, sensors, 4);
