...
/*  Program Description																*/
/*																		*/
/* 	The Pine A64 HAT board uses UART2 (/dev/ttyS2) configured as a Modbus RTU master over an RS485 physical layer, 				*/
/*  	with manual DE/RE direction control handled via a dedicated GPIO pin. 									*/
/*  	The system periodically polls the connected VAV thermostat device and reads all 18 parameter registers at a configurable interval.	*/
/*																		*/
/************************************************************************************************************************************************/
...
int main()
{
  ...
  modbus_set_response_timeout(modbusRtu, 6, 0);
...
int vavThermostat(int i, uint16_t buf[])
{
  ...
  {
    case 0:
      printf("Reg[%d] = %d ---> Device / Slave ID\n",i , buf[i]);
      break;

    case 1:
      printf("Reg[%d] = %d ---> Fan Mode/State 0 = OFF and 1 = ON\n", i, buf[i]);
      break;

    case 2:
      printf("Reg[%d] = %d ---> on / off source. 1 = off and 2 (or) 8 = ON \n", i, buf[i]); 
      break;

    case 3:
      printf("Reg[%d] = %d ---> mode 0 = Auto and 1 = manual (or) Calibration\n", i, buf[i]);
      break;
    ...
    case 6:
      printf("Reg[%d] = %.2f ---> Vnom\n", i, (float)buf[i] / 1.0);
      // printf("Reg[%d] = %1.f \u00B0C ---> Room Temperature\n", i, buf[i] / 10.0);
      break;

    case 7:
      printf("Reg[%d] = %.2f ---> Vmin\n", i, (float)buf[i] / 1.0);
      break;

    case 8:
      printf("Reg[%d] = %.2f ---> Vmax\n", i, (float)buf[i] / 1.0);
      break;

    case 9:
      printf("Reg[%d] = %.2f ---> Delta Pressure\n", i, (float)buf[i] / 100.0);
      break;
    ...
    case 11:
      printf("Reg[%d] = %.2f \u00B0C ---> Set Temperature\n", i, (float)buf[i] / 100.0);
      break;

    case 12:
      printf("Reg[%d] = %.2f \u00B0C ---> Ambient Temperature (Room Temperature)\n", i, (float)buf[i] / 100.0);
      break;

    case 13:
      printf("Reg[%d] = %d ---> PIR Status\n", i, buf[i]);
      break;

    case 14:
      printf("Reg[%d] = %.2f \u00B0C ---> Temperature Offset\n", i, (float)buf[i] / 100.0);
      break;

    case 15:
      printf("Reg[%d] = %d ---> Control Mode (0 = Pressure Dependent, 1 = Pressure Independent)\n", i, buf[i]);
      break;

    case 16:
      printf("Reg[%d] = %d ---> Direction (0 = Forward, 1 = Reverse)\n", i, buf[i]);
      break;

    case 17:
      printf("Reg[%d] = %.1f%% ---> Set Damper position (Commanded / set vlaue)\n", i, (float)buf[i] / 1.0);
      break;
